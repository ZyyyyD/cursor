...
export default function ReportsScreen() {
  ...
  const reports = [
    {
      ...
      onView: () => {
        const byCategory = Object.entries(stats.byCategory)
          .map(
            ([cat, data]) =>
              `${cat}: ${data.items} items, ${data.qty} units, ${formatCurrency(data.value)}`,
          )
          .join("\n");
        Alert.alert(
          "Inventory Summary",
    ...
    },
    {
      ...
      onView: () => {
        ...
        Alert.alert(
          "Low Stock Items",
          lowItems || "All items are well stocked!",
        );
      },
    },
    {
  ...
  const handleExport = (reportTitle) => {
    Alert.alert("Export Report", `Export "${reportTitle}" to XLSX?`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Export",
        onPress: () => Alert.alert("Success", `${reportTitle} exported!`),
      },
    ]);
  };

  const handleExportAll = () => {
    Alert.alert("Export All Data", "Export all inventory and sales data?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Export",
        onPress: () =>
          Alert.alert("Success", "All data exported to Inventory_Export.xlsx"),
      },
    ]);
  };
        ...
        {reports.map((report) => (
          <Card key={report.title} style={styles.reportCard}>
            ...
            <View style={styles.reportActions}>
              ...
              <Pressable
                style={[styles.reportActionBtn, styles.exportBtn]}
                onPress={() => handleExport(report.title)}
              >
        ...
        {stats.productCount === 0 && stats.transactionCount === 0 && (
          <Card style={styles.emptyCard}>
            ...
            <Text style={styles.emptyText}>
              Add inventory items and make sales to see reports
            </Text>
          </Card>
        )}
