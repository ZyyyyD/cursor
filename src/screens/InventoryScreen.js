...
export default function InventoryScreen() {
  const customCategories = useCategoriesStore((state) => state.categories);
  ...
  // Inside InventoryScreen() function:

  // Get categories for UI & stats
  const categories = useMemo(() => {
    const catsMap = {};

    // 1️⃣ Add all custom categories from store (dynamic)
    customCategories.forEach((cat) => {
      const name = cat.name; // your store already has {id, name}
      catsMap[name] = { count: 0, value: 0, isCustom: true };
    });

    // 2️⃣ Aggregate inventory items into categories
    items.forEach((item) => {
      const catName = item.category || "Uncategorized";

      // If this item has a category not in the map, add it dynamically
      if (!catsMap[catName]) {
        catsMap[catName] = { count: 0, value: 0, isCustom: false };
      }

      catsMap[catName].count += 1;
      catsMap[catName].value +=
        (Number(item.qty) || 0) * (Number(item.price) || 0);
    });

    // 3️⃣ Convert map to array for UI
    return Object.entries(catsMap).map(([name, data]) => ({
      name,
      ...data,
    }));
  ...
  const handleAdjustStock = () => {
    ...
    Alert.alert(
      "Success",
      `Stock ${adjustType === "in" ? "added" : "removed"} successfully!`,
    );
  };
        ...
        <View style={styles.resultsRow}>
          ...
          <View style={styles.displayToggle}>
            <Pressable
              style={[
                styles.displayBtn,
                displayMode === "card" && styles.displayBtnActive,
              ]}
            ...
            >
              <Icon
                ...
                size={16}
                color={
                  displayMode === "card" ? colors.surface : colors.textSecondary
                }
              />
            </Pressable>
            <Pressable
              style={[
                styles.displayBtn,
                displayMode === "grid" && styles.displayBtnActive,
              ]}
            ...
            >
              <Icon
                ...
                size={16}
                color={
                  displayMode === "grid" ? colors.surface : colors.textSecondary
                }
              />
            </Pressable>
            <Pressable
              style={[
                styles.displayBtn,
                displayMode === "list" && styles.displayBtnActive,
              ]}
            ...
            >
              <Icon
                ...
                size={16}
                color={
                  displayMode === "list" ? colors.surface : colors.textSecondary
                }
              />
            </Pressable>
          </View>
              ...
              <View style={styles.itemFooter}>
                <View style={styles.qtyWrap}>
                  ...
                  <Text style={styles.qtyText}>
                    <Text style={styles.qtyValue}>{item.qty}</Text> / {item.min}{" "}
                    min
                  </Text>
                </View>
        ...
        {displayMode === "list" && filteredItems.length > 0 && (
          <View style={styles.listContainer}>
            ...
            <View style={styles.listHeader}>
              <Text style={[styles.listHeaderText, styles.listColName]}>
                Item
              </Text>
              <Text style={[styles.listHeaderText, styles.listColQty]}>
                Qty
              </Text>
              <Text style={[styles.listHeaderText, styles.listColPrice]}>
                Price
              </Text>
              <Text style={[styles.listHeaderText, styles.listColStatus]}>
                Status
              </Text>
            </View>
            {/* List Items */}
            {filteredItems.map((item, index) => (
              ...
              >
                ...
                <Text style={[styles.listCellText, styles.listColQty]}>
                  {item.qty}
                </Text>
                <Text style={[styles.listCellText, styles.listColPrice]}>
                  ₱{item.price?.toFixed(2) || "0.00"}
                </Text>
                  ...
                  <View style={styles.categoryRow}>
                    {customCategories.map((cat) => (
                      <Pressable
                        ...
                        style={[
                          styles.categoryChip,
                          newItem.category === cat.name &&
                            styles.categoryChipActive,
                        ]}
                        onPress={() =>
                          setNewItem({ ...newItem, category: cat.name })
                        }
                      >
                        <Text
                          style={[
                    ...
                    {/* Optionally: Add new category inline */}
                    <Pressable
                        ...
                        Alert.prompt(
                          ...
                          (text) => {
                            if (text?.trim()) {
                              ...
                              setNewItem({ ...newItem, category: text.trim() });
                            }
                          },
                        );
                      }}
              ...
              <View style={styles.inputRow}>
                ...
                <View style={[styles.inputGroup, { flex: 1 }]}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) =>
                      setNewItem({ ...newItem, wholesalePrice: text })
                    }
                    placeholder="0.00"
                    placeholderTextColor={colors.textMuted}
                    keyboardType="decimal-pad"
            ...
            {selectedItem && (
              <>
                ...
                <View style={styles.detailRow}>
                  ...
                  <Text style={styles.detailValue}>
                    ₱{selectedItem.wholesalePrice?.toFixed(2) || "0.00"}
                  </Text>
                </View>
                <View style={styles.detailRow}>
                ...
                <View style={styles.detailRow}>
                  ...
                  <Text
                    style={[
                      styles.detailValue,
                      selectedItem.qty <= selectedItem.min && {
                ...
                <View style={styles.detailActions}>
                  <Button
                    ...
                    variant="outline"
                    icon="plus"
                    onPress={() => openAdjustModal(selectedItem, "in")}
                    style={{ flex: 1 }}
                  />
                  <Button
                    title="Stock Out"
                    variant="outline"
                    icon="minus"
                    onPress={() => openAdjustModal(selectedItem, "out")}
                    style={{ flex: 1 }}
                  />
                </View>
          ...
          <View style={styles.modalContent}>
            ...
            {selectedItem && (
              <>
                ...
                <Text style={styles.adjustItemMeta}>
                  Current stock: {selectedItem.qty} units
                </Text>
