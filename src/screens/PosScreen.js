...
export default function PosScreen() {
  ...
  const total =
    cart.reduce((sum, item) => sum + item.qty * item.price, 0) - discountAmount;
  ...
  const handleAddToCart = (product) => {
    ...
    if (currentCartQty >= inventoryItem.qty) {
      Alert.alert(
        "Out of Stock",
        `Only ${inventoryItem.qty} available in stock`,
      );
      return;
    }
          ...
          <View style={styles.headerActions}>
            ...
            >
              <Icon
                name="percent"
                size={20}
                color={discount > 0 ? colors.primary : colors.textSecondary}
              />
            </Pressable>
          </View>
        </View>
