...
export default function OrdersScreen() {
  ...
  const receivedProducts = useReceivedProductsStore(
    (state) => state.receivedProducts,
  );
  const addReceivedProducts = useReceivedProductsStore(
    (state) => state.addReceivedProducts,
  );
  const deleteReceivedProduct = useReceivedProductsStore(
    (state) => state.deleteReceivedProduct,
  );
  const clearAllReceivedProducts = useReceivedProductsStore(
    (state) => state.clearAllReceivedProducts,
  );
  ...
  const totals = useMemo(() => {
    const totalQty = receivedProducts.reduce(
      (sum, item) => sum + item.quantity,
      0,
    );
    const totalAmount = receivedProducts.reduce(
          ...
          <View style={styles.tableContainer}>
            ...
            {receivedProducts.map((item, index) => (
              ...
              >
                ...
                <Text
                  style={[styles.tableCellText, styles.colProduct]}
                  numberOfLines={2}
                >
                  {item.productName}
                </Text>
        ...
        {receivedProducts.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>
              Tap the + button to record incoming stock from suppliers
            </Text>
            <Button
              title="Receive Stock"
              icon="plus"
            ...
            >
                ...
                {quantity && unitPrice && (
                  <View style={styles.subtotalPreview}>
                    ...
                    <Text style={styles.subtotalValue}>
                      ₱
                      {(
                        parseInt(quantity || 0) * parseFloat(unitPrice || 0)
              ...
              <View style={styles.infoBox}>
                ...
                <Text style={styles.infoText}>
                  Products matching inventory names will update stock. New
                ...
                <View style={styles.pendingSection}>
                  ...
                  <View style={styles.pendingTable}>
                    <View style={styles.pendingTableHeader}>
                      <Text style={[styles.pendingHeaderText, { flex: 2.5 }]}>
                        Product
                      </Text>
                      <Text
                        style={[
                    ...
                    {pendingItems.map((item, index) => (
                      <View
                        ...
                        style={[
                          styles.pendingRow,
                          index % 2 === 0 && styles.pendingRowEven,
                        ]}
                      >
                        <Text
                          style={[styles.pendingCellText, { flex: 2.5 }]}
                          numberOfLines={1}
                        >
                          {item.productName}
                        </Text>
                    ...
                    <View style={styles.pendingTotalRow}>
                      <Text style={[styles.pendingTotalLabel, { flex: 2.5 }]}>
                        TOTAL
                      </Text>
                      <Text
                        style={[
                          styles.pendingTotalText,
                          { flex: 1, textAlign: "center" },
                        ]}
                      >
                        {pendingTotals.totalQty}
                      </Text>
                      <Text
                        style={[styles.pendingTotalText, { flex: 1.5 }]}
                      ></Text>
                      <Text
                        style={[
                          styles.pendingTotalAmount,
                          { flex: 1.5, textAlign: "right" },
                        ]}
                      >
                        ₱
                        {pendingTotals.totalAmount.toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                        })}
                      </Text>
                      <View style={{ width: 32 }} />
